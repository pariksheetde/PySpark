...
def process_fifa_stats_agg():
    ...
    agg_goals_year_df = sel_fifa.groupBy("Year", "Home_Team_Name") \
        .agg(
    ...
    sum_goals_home_team_df = sel_fifa.groupBy("Year", "Home_Team_Name") \
      .agg(
            sum("Goals").alias("Sum_Goals")
          ) \
      .sort(col("Year").asc(), col("Sum_Goals").desc())

    return sum_goals_home_team_df

if __name__ == "__main__":
    spark = SparkSession.builder.appName("FIFA Stats Agg 2").master("local[3]").getOrCreate()

    # Suppress unnecessary Spark logging
    spark.sparkContext.setLogLevel("ERROR")

    result_df = process_fifa_stats_agg()
