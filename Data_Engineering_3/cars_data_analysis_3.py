...
if __name__ == "__main__":
    print("Package : Data_Engineering_3, Script : Cars Data Analysis 3")

spark = SparkSession.builder.appName("Cars Data Analysis 3").master("local[3]").getOrCreate()
...
origin_cnt = cars_df.select(col("Origin")) \
    .groupBy(col("Origin")) \
    .agg(
      count("Origin").alias("Cnt_Origin")
    )
origin_cnt.show(truncate=False)

print(f"Compute year, month, dd")
